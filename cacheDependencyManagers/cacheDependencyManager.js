...
CacheDependencyManager.prototype.installCachedDependencies = function (cachePath, compressedCacheExists, callback) {
    ...
    if (compressedCacheExists) {
        fs.createReadStream(cachePath)
            ...
            .on('error', onError)
            .on('finish', onEnd);
    } else if (self.config.createSymlink) {

        var target = cachePath + '/' + self.config.installDirectory;
        self.cacheLogInfo('will create link to ' + target + ' at ' + installDirectory);
        fs.symlink(target, installDirectory, function (err) {
            if (err) {
                onError(err);
            } else {
                onEnd();
